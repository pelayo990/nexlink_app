    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', background: '#fff', borderRadius: 16, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
        ...
        <div style={{ padding: 28 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, fontSize: 13, flexWrap: 'wrap' }}>
    ...
    <>
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ height: 160, background: 'linear-gradient(135deg, #EEF2FF, #E0E7FF)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 64, position: 'relative', overflow: 'hidden' }}>
          ...
          {lowStock && (
            <span style={{ position: 'absolute', top: 12, right: 12, background: '#FEF3C7', color: '#92400E', padding: '3px 8px', borderRadius: 99, fontSize: 11, fontWeight: 600 }}>
              ¡Últimas unidades!
            </span>
          )}
        </div>
        ...
        <div style={{ padding: 16 }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 4 }}>
            {producto.categoria} • {producto.empresa?.nombre || producto.marca?.nombre}
          </div>
          ...
          <div style={{ display: 'flex', gap: 0 }}>
            <button onClick={() => setShowModal(true)}
              style={{ flex: 1, height: 40, background: 'var(--bg)', border: '1px solid var(--border)', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', borderRadius: '0 0 0 8px', borderRight: 'none' }}>
              Ver detalle
            </button>

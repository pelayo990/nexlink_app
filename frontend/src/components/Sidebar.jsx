...
const NAV = {
  ...
  empresa: [
    ...
    { to: '/empresa/productos', icon: Package, label: 'Mis Productos' },
  ...
  return (
    ...
    style={({ isActive }) => ({
      ...
      <div style={{ padding: '16px 12px', borderTop: '1px solid rgba(255,255,255,.08)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 8, background: 'rgba(255,255,255,.04)' }}>
          <div style={{ width: 34, height: 34, borderRadius: '50%', background: ROL_COLORS[user?.rol] || 'var(--primary-gradient)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
          ...
          <button onClick={handleLogout} title="Cerrar sesión" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#64748B', padding: 4, borderRadius: 4 }}>
            <LogOut size={16} />
          </button>
        </div>
      </div>
